...
rmvec<-unlist(lapply(c("none","not relevant","not behavior","not a behavior","not quantitative genetics","meeting abstract",
                       "not useful","plant","no abstract","just the abstract"),
              grep,x=qgb$Relevance.to.article))
qgb<-qgb[-rmvec,]
```
...
```{r categories}
cats<-apply(exp[,15:(ncol(exp)-1)],2,function(c){ length(c[c!=""])})
...
png("Personality_Timeline.png",height=4,width=6,units="in",res=300)
par(mar=c(4,4,2,2))
...
![Personality Timeline](../qgb_data/Personality_Timeline.png)
